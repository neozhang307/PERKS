    ...
    while((i<len(contents))):
        ...
        if m and arch:
            if(re.search(funcname,m.group(1))):
                ...
                # return spillnum
                return arch.group(0),Type, regusage,smusage,storespill,loadspill
            i+=4
        else:
            i+=1
        # i+=1
...
def singletask(filename,halostring,realstring,box,archstring,asyncSM):
    regfolder=0
    file = open(filename, 'a')
    original_out=sys.stdout
    # while (True):
    # for regfolder in range(3):
    subprocess_return = compile(archstring,halostring,realstring,asyncSM,box)
    print(subprocess_return)
    returnlist =parseptx(subprocess_return.splitlines(), "baseline")
    ...
    sys.stdout = file 
    print("kernel_baseline",halostring,returnlist[0],returnlist[1],returnlist[2],returnlist[3],returnlist[4],returnlist[5])
...
Hrange("bstar_80.log",1,6,"","arch=compute_80,code=sm_80","")
Hrange("bstar_80_async.log",1,6,"","arch=compute_80,code=sm_80",",ASYNCSM")
Hrange("bstar_70.log",1,6,"","arch=compute_70,code=sm_70","")

Hrange("bbox_80.log",1,2,",BOX","arch=compute_80,code=sm_80","")
Hrange("bbox_80_async.log",1,2,",BOX","arch=compute_80,code=sm_80",",ASYNCSM")
Hrange("bbox_70.log",1,2,",BOX","arch=compute_70,code=sm_70","")
