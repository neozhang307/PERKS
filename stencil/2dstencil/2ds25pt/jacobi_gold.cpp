...
(const REAL* l_input, int width_y, int width_x, REAL* l_output, int step)
{
  ...
  REAL (*output)[width_x] = (REAL (*)[width_x])l_output;
  for( int i = 6; i < width_y-6; i++ )
    for( int j = 6 ; j < width_x-6; j++ )
      output[i][j] = (
        2*input[i-6][j] + 
        6*input[i-5][j] + 
        1*input[i-4][j] + 
        ...
        7*input[i-2][j] + 
        5*input[i-1][j] + 
        6*input[i][j-6] + 5*input[i][j-5] + 2*input[i][j-4] + 3*input[i][j-3] + 9*input[i][j-2] + 12*input[i][j-1] + 
        15*input[i][j] + 
        12*input[i][j+1] + 9*input[i][j+2] + 3*input[i][j+3]+  3*input[i][j+4]+ 4*input[i][j+5]+ 6*input[i][j+6]+ 
        5*input[i+1][j] + 
        7*input[i+2][j] + 
        5*input[i+3][j] + 
        4*input[i+4][j] + 
